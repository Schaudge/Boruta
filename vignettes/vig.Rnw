...
In particular, there is one for rFerns, an implementation of random ferns, a purely stochastic ensemble classifier which can usually provide similar importance scores as random forest, but in substantially shorter time:
...
Whenever possible, try to assess its results, especially in terms of selection stability as classification accuracy may be deceiving.
